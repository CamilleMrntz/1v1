...
function movePlayer() {
    ...
    if (keys[38] && player.x < canvas.width) {
    ...
    // Attack SPACE
    if (keys[32] && player.x < canvas.width - player.speed) {
        // player.x += player.speed;
        player.frameY = 1;
    ...
    // Enemy side
    // Go left Q
    if (keys[81] && enemy.x > 0) {
        enemy.x -= enemy.speed;
        enemy.frameY = 0;
    }
    // Go right D
    if (keys[68] && enemy.x < canvas.width) {
        enemy.x += enemy.speed;
        enemy.frameY = 2;
    }
    // Go left faster Z
    if (keys[90] && enemy.x > 0) {
        enemy.x -= enemy.speed + 10;
        enemy.frameY = 0;
    }
    // Go right faster S
    if (keys[83] && enemy.x < canvas.width) {
        enemy.x += enemy.speed + 10;
        enemy.frameY = 2;
    }
    // Attack R
    if (keys[82] && enemy.x < canvas.width - enemy.speed) {
        enemy.frameY = 1;
